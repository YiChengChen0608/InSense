  ...
  return (
    <>
      ...
      <MainContainer otherClass="class-list-container d-flex flex-wrap justify-content-center">
        {classInfo.map(info => <ClassCard key={info.classId} classImg={info.classImg} classTime={info.classTime} className={info.className} classId={info.classId} remainingPeople={info.remainingPeople} logInStatus={user.logInStatus} bookInfo={bookInfo} userId={user.userInfo.userId} />)}
      </MainContainer>
    </>
  );
};
